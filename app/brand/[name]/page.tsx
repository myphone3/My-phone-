...
export default function BrandPage() {
  ...
  const fetchData = async () => {
    ...
    const [prodRes, brandRes] = await Promise.all([
      supabase.from('products').select('*').eq('brand', brandName).or('is_published.is.null,is_published.eq.true'),
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-6 space-y-6" dir="rtl">
      ...
      {/* סרגל ניווט מהיר בין מותגים */}
      {brands.length > 0 && (
        <div className="overflow-x-auto pb-2 scrollbar-none">
          <div className="flex items-center gap-4 w-max">
            {brands.map((brand) => {
              const isActive = brand.name === brandName;
              return (
                <Link
                  key={brand.id}
                ...
                >
                  {brand.image_url && <img src={brand.image_url} alt="" className="w-6 h-5 object-contain" />}
      ...
      <div className="border-b pb-4 flex justify-between items-center">
        <h1 className="text-xl sm:text-2xl font-black text-gray-900">מותג: {brandName}</h1>
      ...
      {loading ? (
        ...
        <div className="text-center py-20 bg-white rounded-3xl border p-8 space-y-3 shadow-sm">
          ...
          <p className="text-gray-500 font-medium">אין מוצרים זמינים למותג זה כרגע.</p>
